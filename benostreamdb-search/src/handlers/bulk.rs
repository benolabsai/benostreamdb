...
fn parse_bulk(body: &str, default_index: Option<&str>) -> Result<Vec<BulkItem>, BenoStreamError> {
    ...
    while i < lines.len() {
        ...
        raw_actions.push(RawAction {
            action,
            source_line,
            line_num: i + 1 - if source_line.is_some() { 1 } else { 0 },
        });
        i += 1;
    }
...
async fn write_index_docs(
    ...
    for (_pos, _item, s) in &doc_schemas[1..] {
        match infer::merge_schemas(merged.as_ref(), s.as_ref()) {
            Ok(m) => merged = std::sync::Arc::new(m),
            Err(e) => {
    ...
    for (pos, item, _s) in &doc_schemas {
        let Some(mut doc) = item.doc.clone() else {
    ...
    let wal_ms = t_wal.elapsed().as_millis();
    tracing::debug!(
        index,
        docs = items.len(),
        infer_ms,
        batch_ms,
        wal_ms,
        "write_index_docs timing"
    );
    results
}

/// Map a typed write error to its ES status code.
fn es_status(err: &BenoStreamError) -> u16 {
    match err {
        BenoStreamError::PrimaryKeyViolation { .. } => 400,
        BenoStreamError::NullConstraintViolation { .. } => 400,
        BenoStreamError::SchemaIncompatible { .. } => 400,
        _ => 500,
    }
}
