...
HyperStreamDB is a high-performance, indexed streaming lakehouse built in Rust. It combines authoritative open table storage (Apache Iceberg V2 & V3) with reconstructible persistent index overlays (Roaring Bitmaps, BM25 Okapi, and HNSW vector search) directly on object storage.
...
*   **Dual REST Search API (`hypersearch`)**: Drop-in OpenSearch / Elasticsearch 7.10 compatibility (port 9200) and Qdrant Vector API emulation (port 6333) from a single server.
...
Check out the [Architecture](architecture.md) guide to understand how HyperStreamDB works, the [Comprehensive Guide](COMPREHENSIVE_GUIDE.md) for feature overviews, or the [Benchmarking Guide](BENCHMARKING.md) for competitive performance analysis.
