...
This document provides a comprehensive overview of HyperStreamDB's benchmarking methodology, competitive evaluation against **OpenSearch 2.11** at 100K and 1M document scales, comparison against LanceDB, and Rust-native micro-benchmark test suites.

---

## 1. Competitive Benchmarks: `hypersearch` vs. OpenSearch 2.11 (100K & 1M Docs)

This section consolidates the empirical findings from our long-running competitive scaling benchmarks comparing HyperStreamDB's `hypersearch` REST server against a single-node OpenSearch 2.11 instance.
...
| Scale | Operation / Metric | HyperStreamDB (p50) | HyperStreamDB (p99) | OpenSearch 2.11 (p50) | OpenSearch 2.11 (p99) | Performance Ratio |
| :--- | :--- | :--- | :--- | :--- | :--- | :--- |
...
| **100K** | `knn` HNSW Vector Search (ms) | **1.94** | **4.26** | 4.29 | 62.58 | **HyperStreamDB 14.7x faster p99** |
| **100K** | Operational Storage Footprint | **~26.0 MB** | — | ~185.4 MB | — | **HyperStreamDB 7.1x less disk** |
| **1M** | Ingest Throughput (docs/s) | 4,613 | — | **9,221** | — | 0.50x *(deferred index build in OS)* |
| **1M** | `knn` HNSW Vector Search (ms) | **1.91** | **3.74** | 8.26 | 478.77 | **HyperStreamDB 128x faster p99** |
| **1M** | Operational Storage Footprint | **~280 MB** | — | ~1,852 MB (1.85 GB) | — | **HyperStreamDB 6.6x less disk** |
...
- **100K Scale**: HyperStreamDB achieves a p50 of **1.94 ms** and p99 of **4.26 ms**, outperforming OpenSearch's 4.29 ms p50 and 62.58 ms p99.
- **1M Scale**: HyperStreamDB's query latency remains **completely flat** at **1.91 ms p50** and **3.74 ms p99**. Conversely, OpenSearch suffers catastrophic tail latency collapse: its p50 degrades to **8.26 ms**, while its p99 explodes to **478.77 ms** (nearly half a second).
...
- **HyperStreamDB (Native Lakehouse Overlay Architecture)**:
  - HyperStreamDB **is** the Data Lake. It directly queries native Parquet data and creates compact `.hnsw` sidecar index files alongside data files.
  - At 100K vectors, HyperStreamDB requires **~26.0 MB** total storage (**7.1x storage savings**).
  - At 1M vectors, HyperStreamDB requires **~280 MB** total storage (**6.6x storage savings**).
...
- **HyperStreamDB**: Built as a stateless Rust binary. It attaches directly to the Parquet and `.hnsw` sidecars via memory mapping (`mmap`) backed by the OS page cache. Upon process restart or scale-to-zero wake-up, queries are served almost instantly with zero warm-up penalty.
...
- HyperStreamDB's baseline synchronous indexing builds graph structures in the hot path (~4.4k to ~4.6k docs/s).
- With the introduction of the Async WAL and Memtable batch coalescing pipeline, HyperStreamDB decouples physical ingestion from background HNSW construction.
...
HyperStreamDB features high-throughput vector ingestion that rivals dedicated local-first vector formats like LanceDB, while preserving full Apache Iceberg table format compatibility.
...
### Competitive Ingest Comparison: HyperStreamDB vs. LanceDB (100k 768D Vectors)
- **HyperStreamDB (768D, 100k)**: **22,999 rows/sec**
...
- **OpenSearch / Elasticsearch**: Primary targets for the REST Search API (`hypersearch`). Evaluated on search latency (BM25, kNN, hybrid RRF), index footprint, and cold starts.
...
For low-level execution without Python runtime overhead, HyperStreamDB includes a comprehensive suite of native micro-benchmarks implemented with [Criterion](https://github.com/bheisler/criterion.rs):
