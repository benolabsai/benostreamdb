...
In addition to catalog-level atomicity, HyperStreamDB includes a built-in cloud-agnostic distributed lock mechanism (`FileBasedLock`):
...
HyperStreamDB provides configurable durability policies for streaming writes (`HYPERSTREAM_WAL_DURABILITY`):
...
- **`periodic`**: Asynchronously flushes WAL buffers on a timer (configured via `HYPERSEARCH_AUTO_REFRESH_SECS`).
...
HyperStreamDB decouples I/O stream concurrency from CPU compute parallelism:

1. **I/O Pipeline (`HYPERSTREAM_MAX_CONCURRENCY`)**: Tokio asynchronously pulls up to $N$ Parquet segments concurrently via non-blocking streams (`buffer_unordered`). Non-blocking tasks spend the majority of their time awaiting network packets or NVMe block reads without consuming CPU cores.
...
| Deployment Profile | Machine Specs | `RAYON_NUM_THREADS` (Compute) | `HYPERSTREAM_MAX_CONCURRENCY` (I/O) | Focus |
|:---|:---|:---|:---|:---|
