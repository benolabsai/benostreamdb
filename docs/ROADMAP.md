...
> **⚠️ Performance figures below are historical.** Every benchmark in this
> document other than the full-site Wikipedia demo
> ([`examples/web_ui/README.md`](../examples/web_ui/README.md)) is out of date —
> the OpenSearch / LanceDB comparisons, the NYC-Taxi runs, and the Criterion
> micro-benchmarks have been removed from the repository. Do not cite them.
...
- **API & SQL Guides**:
  ...
  - `CONCURRENCY.md` & `architecture.md` — Concurrency model and architecture breakdown.
- **Service Quickstarts**:
...
- [x] **A5.3 — Native Metal correctness**: added an `n_vectors` argument + bounds guard to all six Metal distance kernels (the dispatch covers `ceil(n/256)*256` threads, so up to 255 threads read past the end of the vector buffer), and aligned the Hamming (`!=`) and Jaccard (`> 0.0`) comparisons with the CPU/CUDA definition. A new `metal` CI job runs the cross-backend harness on `macos-14` (Apple Silicon), so this is verified in CI rather than needing a local Mac. ✅
