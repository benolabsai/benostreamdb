...
export BENOSTREAM_STORAGE_URI="$TEST_URI"
...
cargo run --bin benostreamdb-admin -- compact --uri "$TEST_URI" --target-file-size 10MB

echo "3. Testing Vacuum..."
cargo run --bin benostreamdb-admin -- vacuum --uri "$TEST_URI" --retain-versions 1
