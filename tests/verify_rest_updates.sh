...
# Ensure clean state
rm -rf /tmp/benostream_rest_test
...
curl -v -X POST http://127.0.0.1:8181/v1/bsdb/namespaces/default/tables/test_table \
  -H "Content-Type: application/json" \
...
RESPONSE=$(curl -s http://127.0.0.1:8181/v1/bsdb/namespaces/default/tables/test_table)
