...
class IndexType:
    """
    BenoStreamDB Indexing Algorithms.
    """
    HNSW = "hnsw"
    BM25 = "bm25"
    BLOOM = "bloom"
...
def open_table(uri: str, **kwargs) -> Table:
    """Open an existing BenoStreamDB table."""
    return Table(uri, **kwargs)
class Query:
    """
    Fluent Query interface for BenoStreamDB.
    """
...
class Table:
    """
    BenoStreamDB Table — Apache Iceberg/Parquet-compatible columnar vector store.
    ...
    **Default behaviour (v0.4.1+)**
    ...
    def to_networkx(
        ...
        """
        Export this BenoStreamDB edge table to a NetworkX Graph, DiGraph, MultiGraph, or MultiDiGraph.
    ...
    def upsert(self, data: Any, key_column: Union[str, List[str]], mode: str = "merge_on_read", device: Optional[Any] = None):
        ...
        from .benostreamdb import PyMergeMode
    ...
    def ingest(
        ...
        Args:
            ...
            memory_budget_gb: return freed heap pages to the OS after a unit once
                ...
                ``BSDB_INGEST_MEMORY_BUDGET_GB`` env var when unset; disabled if
                neither is set.
    ...
    def __repr__(self):
        return f"BenoStreamTable(uri={self._inner.table_uri()})"
