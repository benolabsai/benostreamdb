...
**Next Steps**
- [ ] **`BETWEEN` still does not prune** *(narrow follow-up)*: the `Expr::Between` arm exists but the planner's `FilterExpr::parse_sql` path does not reach it — comparison predicates (`>=`, `=`, `>`) prune correctly. Next step: dump the parsed expression for `id BETWEEN 25 AND 32` and see which node it actually is.
- [ ] **Surface the pruning breakdown in the DataFusion plan**: pruning reasons currently only appear in the engine's own `explain()`; emitting them from the custom exec node would make one EXPLAIN show everything.
