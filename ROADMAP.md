...
- [ ] **Empty `column_stats` makes stats pruning inert** *(partially fixed — chain isolated)*. The full chain is: parquet statistics were **disabled** → `merge_parquet_stats` had nothing to read → `ManifestEntry.column_stats` empty → the Avro manifest writer hardcoded `lower_bounds`/`upper_bounds`/`null_value_counts` to **Null** → the reader rebuilds `column_stats` from exactly those three (and only when all three are present) → nothing → `classify_condition`'s stats branch can never fire. Two links are now fixed: `EnabledStatistics::Chunk` (was `None`) so parquet carries chunk min/max, and `bounds_avro_values` + `encode_iceberg_value` so real bounds/null-counts are written. **Still empty end-to-end**, which isolates the remaining link to `column_stats` being empty by the time the manifest is written — most likely the background-indexing path: `write.rs` re-derives the entry from `index_writer.to_manifest_entry()`, and that writer only builds indexes, so it never called `write_batch`/`merge_parquet_stats` and its (empty) stats map replaces the real one. Next step: verify that and carry the data-writer's stats into the index-updated entry.
