...
**Next Steps**
- [ ] **Sparse Vectors → DataFusion representation**: Map/Struct support in the vector-search sort-expr parser (`src/core/sql/optimizer/vector_search/sort_expr_parser.rs`). *(Arrow IPC serialization is now done — see Completed.)*
- [ ] **Vector Search I/O — score-only short-circuit**: `read_rows_by_id` now honours the column projection (it previously read every column regardless), but a query that needs *only* the score still fetches rows. Short-circuit it by emitting the score straight from `ScoredResult` when the projection is just `_distance`/`score`.
