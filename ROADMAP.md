...
- [ ] **Empty `column_stats` makes stats pruning inert** *(narrowed to one hop)*. Chain, each link now measured rather than inferred:
  1. parquet statistics were **disabled** (`EnabledStatistics::None`) → **fixed** (`Chunk`).
  2. the Avro manifest writer hardcoded `lower_bounds`/`upper_bounds`/`null_value_counts` to Null, and the reader rebuilds `column_stats` from exactly those three → **fixed** (`encode_iceberg_value` + `bounds_avro_values`).
  3. `HybridSegmentWriter::write_batch` **does** populate `column_stats` — proven by `write_batch_populates_column_stats` (`id min=40 max=42`).
  4. `ArrowWriter::close()` metadata **does** carry chunk statistics — proven by `parquet_close_metadata_carries_chunk_statistics`.
  **Remaining bug:** instrumenting `bounds_avro_values` shows **two** `write_manifest_chunks` calls per `Table::commit()` — one with `stats=2 / lowers=2`, then one with `stats=0 / lowers=0`. The stats-less manifest is the one the reader ends up seeing. `write_manifest_chunks` has exactly one caller (`commit.rs:107`), so there are two `ManifestManager::commit` calls per flush — `write.rs:614` (`commit_async()`) and `write.rs:1149` (`commit(&all_new_entries, ...)`). Next step: determine which of those carries entries lacking `column_stats` and either give it stats or stop it overwriting the good manifest.
