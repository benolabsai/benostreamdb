...
- [x] **`column_stats` was empty → statistics pruning was inert** — **FIXED**. Four independent faults, any one of which was sufficient:
  ...
  3. **The reader failed to unwrap Avro's nullable-union wrapper.** `parse_map_int_long` / `parse_map_int_bytes` matched a bare `AvroValue::Array`, but a `["null", T]` field decodes as `Union(1, Box(Array(..)))` — so every stats map silently parsed as `None`. → `unwrap_nullable_union`.
...
- [ ] **`BETWEEN` still does not prune** *(narrow follow-up)*: the new `Expr::Between` arm exists but the planner's `FilterExpr::parse_sql` path for `BETWEEN` does not reach it — comparison predicates (`>=`, `=`, `>`) prune correctly. Next step: dump the parsed expression for `id BETWEEN 25 AND 32` and see which node it actually is.
