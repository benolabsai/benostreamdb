use anyhow::Result;
use arrow::array::Int64Array;
use datafusion::prelude::*;
use std::fs;
use std::path::Path;

pub async fn compute_connected_components(
    ctx: &SessionContext,
    edges_table: &str,
    temp_dir: &Path,
    max_iterations: usize,
    src_column: &str,
    dst_column: &str,
) -> Result<String> {
    fs::create_dir_all(temp_dir)?;

    // 1. Initialize State
    let init_sql = format!(
        "
        SELECT id, id AS component_id FROM (
            SELECT {src_column} AS id FROM {edges_table}
            UNION
            SELECT {dst_column} AS id FROM {edges_table}
        )
    "
    );

    let init_df = ctx.sql(&init_sql).await?;
    let init_path = temp_dir.join("cc_0.parquet");
    init_df
        .write_parquet(init_path.to_str().unwrap(), Default::default(), None)
        .await?;

    // Register init state
    ctx.register_parquet(
        "cc_0",
        init_path.to_str().unwrap(),
        ParquetReadOptions::default(),
    )
    .await?;

    let mut current_table = "cc_0".to_string();

    // 2. Iterative Label Propagation
    for i in 0..max_iterations {
        let next_table = format!("cc_{}", i + 1);
        let next_path = temp_dir.join(format!("{}.parquet", next_table));

        let update_sql = format!(
            "
            SELECT 
                node_id AS id,
                MIN(component_id) AS component_id
            FROM (
                SELECT id AS node_id, component_id FROM {current_table}
                UNION ALL
                SELECT e.{dst_column} AS node_id, c.component_id
                FROM {edges_table} e JOIN {current_table} c ON e.{src_column} = c.id
                UNION ALL
                SELECT e.{src_column} AS node_id, c.component_id
                FROM {edges_table} e JOIN {current_table} c ON e.{dst_column} = c.id
            )
            GROUP BY node_id
        "
        );

        let df = ctx.sql(&update_sql).await?;
        df.write_parquet(next_path.to_str().unwrap(), Default::default(), None)
            .await?;

        ctx.register_parquet(
            &next_table,
            next_path.to_str().unwrap(),
            ParquetReadOptions::default(),
        )
        .await?;

        // 3. Convergence Check
        let check_sql = format!(
            "
            SELECT COUNT(*) AS changes
            FROM {next_table} n
            JOIN {current_table} c ON n.id = c.id
            WHERE n.component_id < c.component_id
        "
        );

        let check_df = ctx.sql(&check_sql).await?;
        let batches = check_df.collect().await?;
        let changes_array = batches[0]
            .column(0)
            .as_any()
            .downcast_ref::<Int64Array>()
            .unwrap();
        let changes = changes_array.value(0);

        if changes == 0 {
            return Ok(next_table); // Converged
        }

        current_table = next_table;
    }

    Ok(current_table)
}
