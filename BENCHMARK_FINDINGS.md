# Benchmark Findings — `bsdb-search` vs Elasticsearch 7.10.2

Step 4.2 of the OpenSearch/ES integration plan. Compares the `bsdb-search`
...
- **Raw results:** `benchmarks/competitive/benchmark_results/es710_bsdb-search_*.json` / `.md`

## How to run

```bash
# From the repo root, after building the binary:
cargo build -p benostreamdb-search --bin bsdb-search
...
# bsdb-search-only (no ES) or keep the ES container for inspection:
...
| Operation | BenoStreamDB p50 | BenoStreamDB p95 | ES 7.10.2 p50 | ES 7.10.2 p95 |
|-----------|-------------------|-------------------|---------------|---------------|
...
¹ ES 7.10 has no `dense_vector` type, so `knn` and hybrid are bsdb-search-only.
...
3. **Ingest throughput favors bsdb-search** (879 vs 221 docs/s) for single-doc
...
4. **Refresh (index build) is the one-time cost.** Hypersearch's refresh
   ...
   (46 ms) only flushes the translog. As document count grows, bsdb-search's
...
5. **`knn` and hybrid are differentiators.** ES 7.10 cannot do native vector
   search; bsdb-search serves `knn` and RRF-fused hybrid in the same 50–200 ms
...
observed bsdb-search rate, ~75 min for ES, plus index builds and query runs)
...
`BENOSEARCH_STORAGE_URI=s3://<bucket>/search` with a running MinIO and the
same commands; the on-demand index-file cache
([`benostreamdb-search/src/index_cache.rs`](benostreamdb-search/src/index_cache.rs))
