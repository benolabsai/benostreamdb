...
class MockBenoStreamCatalog extends TableCatalog {
...
class SparkMergeIntegrationTest {
  ...
  def setup(): Unit = {
    spark = SparkSession.builder()
      .master("local[2]")
      .appName("BenoStreamMergeTest")
      .config("spark.sql.catalogImplementation", "in-memory")
      .config("spark.sql.extensions", "org.apache.iceberg.spark.extensions.IcebergSparkSessionExtensions")
      .config("spark.sql.catalog.spark_catalog", "org.apache.iceberg.spark.SparkCatalog")
      .config("spark.sql.catalog.spark_catalog.type", "hadoop")
      .config("spark.sql.catalog.spark_catalog.warehouse", "target/warehouse")
      .config("spark.sql.catalog.mock_hs", "com.benostreamdb.spark.MockBenoStreamCatalog")
      .config("spark.sql.catalog.hs_catalog", "com.benostreamdb.spark.BenoStreamProcedureCatalog")
      .getOrCreate()
  }
  ...
  def testMergeIntoDelegation(): Unit = {
    ...
    // 3. Wrap it in BenoStreamTable and set it in Mock Catalog
    ...
    val hsTable = new BenoStreamTable(icebergTable, icebergTable.schema(), new java.util.HashMap[String, String](), "auto") {
      override def name(): String = "mock_hs.default.users"
    }
    val mockCatalog = spark.sessionState.catalogManager.catalog("mock_hs").asInstanceOf[MockBenoStreamCatalog]
    mockCatalog.setTable(hsTable)
    
    // 4. Verify BenoStreamTable supports RowLevelOperations
    ...
    import com.benostreamdb.spark.BenoStreamRowLevelOperation
    val hsRowOp = rowLevelOp.asInstanceOf[BenoStreamRowLevelOperation]
