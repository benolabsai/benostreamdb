...
\title{\textbf{Out-of-Core Dynamic Reasoning and Inference: Scaling Hybrid Graph Traversal to Terabyte-Scale Vector Databases}}
\author{
    BenoStreamDB Team \\
...
Despite its analytical capabilities, GraphRAG struggles with scalability in production environments due to the "Memory Wall". While modern vector databases have achieved highly efficient out-of-core semantic search (e.g., using disk-backed HNSW graphs) capable of handling very large dense-embedding corpora, graph databases face challenges scaling at the same rate. Graph traversal operations still predominantly require loading network data structures entirely into RAM (e.g., NetworkX, petgraph). For massive datasets, this translates to terabytes of required memory. Partitioning the graph across a distributed database cluster introduces prohibitive network latencies during multi-hop traversals, limiting real-time inference capabilities. A promising approach involves applying the out-of-core indexing paradigms prevalent in vector databases to graph traversal.

\subsection{Our Contribution}
We introduce BenoStreamDB's unified Hybrid Transactional/Analytical Processing (HTAP) engine. This system brings the architectural strengths of out-of-core vector search to bear on GraphRAG. It combines out-of-core global semantic search via an existing hardware-accelerated memory-mapped HNSW with zero-copy local graph traversal via a newly developed Property-Aware CSR memory map. By linking relational storage, memory-mapped vector indices, and memory-mapped graph topologies, we propose an architecture capable of out-of-core DRIFT search that scales to terabyte-scale data on a single node, with larger corpora fanned out across a distributed query engine.
