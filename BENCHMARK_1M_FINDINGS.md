...
2. **BenoStreamDB Ironclad Stability**: Even at 10x the scale, BenoStreamDB's query latency remained **completely flat**. Thanks to the Hot Row Cache bypassing disk I/O, BenoStreamDB achieved a blistering **1.91ms p50** and an ultra-stable **3.74ms p99**. 
3. **Ingest Throughput**: OpenSearch achieves roughly double the ingestion throughput (~9.2k docs/s vs ~4.6k docs/s) because it defers the heavy HNSW graph operations to background merges, while BenoStreamDB builds its graph synchronously in the hot path. 
...
- **BenoStreamDB Total Storage Required**: **~280 MB**
